...
public class StationListAdapter extends RecyclerView.Adapter<StationListAdapter.ViewHolder> implements Filterable {
    ...
    private ArrayList<PlacesResultsItem> modelStation = new ArrayList<>();
    private ArrayList<PlacesResultsItem> modelStationFiltered = new ArrayList<>();
    private List<PhotosItem> stationPhotoItems = new ArrayList<>();
    ...
    public void setData(ArrayList<PlacesResultsItem> items) {
        modelStation.clear();
        modelStation.addAll(items);
        notifyDataSetChanged();

        modelStationFiltered.clear();
        modelStationFiltered.addAll(items);
        notifyDataSetChanged();
    }
    ...
    @Override
    public void onBindViewHolder(@NonNull StationListAdapter.ViewHolder holder, final int position) {
        holder.tvHotelName.setText(modelStationFiltered.get(position).getName());
        holder.tvHotelAddress.setText(modelStationFiltered.get(position).getVicinity());
        stationPhotoItems = modelStationFiltered.get(position).getPhotos();
        int maxWidth = stationPhotoItems.get(0).getWidth();
        String photoReference = stationPhotoItems.get(0).getPhotoReference();
        ...
        if (modelStationFiltered.get(position).getOpeningHours().isOpenNow())
            holder.tvHotelOpenStatus.setText("Open");
        else holder.tvHotelOpenStatus.setText("Closed");

        double rating = modelStationFiltered.get(position).getRating();
        ...
        final String url = "https://www.google.com/search?q=".concat(modelStationFiltered.get(position).getName());
    ...
    public int getItemCount() {
        return modelStationFiltered.size();
    }
    ...
    public Filter getFilter() {
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                if (Key.isEmpty()) {
                    modelStationFiltered = modelStation;
                } else {
                    ArrayList<PlacesResultsItem> listFiltered = new ArrayList<>();
                    for (PlacesResultsItem row : modelStation) {
                        if (row.getName().toLowerCase().contains(Key.toLowerCase())) {
                            listFiltered.add(row);
                        }
                    }
                    modelStationFiltered = listFiltered;
                }

                FilterResults results = new FilterResults();
                results.values = modelStationFiltered;
                return results;
            }
            ...
            protected void publishResults(CharSequence constraint, FilterResults results) {
                ...
                modelStationFiltered = (ArrayList<PlacesResultsItem>) results.values;
                notifyDataSetChanged();
